...
function StudAttendas() {
  ...
  */
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/stud/list-student")
      .then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(
      `http://localhost:8080/stud/add-attendas`, formValues,id)
      .then(res => {
        if(res.status ===200){
...
<form onSubmit={handleSubmit} >
            ...
            <td> 
            <Link className="edit-link" to={`/addAttendass/${data._id}`}>
                   <button  style={{background: "teal",color:"white",padding:"7px",borderRadius:"7px",}}>Add Attendas</button></Link> </td>
             
             
                   <td> 
            <Link className="edit-link" to={`/Result/${data._id}`}>
                   <button  style={{background: "teal",color:"white",padding:"7px",borderRadius:"7px",}}>Add Reult</button></Link> </td> 
                   <td>
                   <Link className="edit-link" to={`/AddCourse/${data._id}`}>
                   <button  style={{background: "teal",color:"white",padding:"7px",borderRadius:"7px",}}>Add Course</button></Link> </td> 
                   </tr>
