...
function EditInstructor(){
  ...
  */
  ...
  useEffect(() =>{
    ...
    axios.get(`http://localhost:8080/api/inst/update-instructor/${id}`)
          .then((res) =>{
  ...
  const handleSubmit = (event) => {
    ...
    axios.put(
      `http://localhost:8080/api/inst/update-instructor/${id}`, formValues)
      .then(res => {
        if(res.status ===200){
          ...
          <input
				...
				<label className={styles.ll_btn}>Class Enroluennt</label>
						<input
							...
							name="cenrolument"
							onChange={handleChange}
							defaultValue={applicant.cenrolment}
							required
							className={styles.input}
